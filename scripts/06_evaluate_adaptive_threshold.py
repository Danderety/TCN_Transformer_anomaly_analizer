...
def main(config_path):
    ...
    va=load_json(f"{c['data']['splits_dir']}/val.json"); va, val_source=combine_validation_with_synthetic_if_needed(c, va); te=load_json(f"{c['data']['splits_dir']}/test.json")
    ...
    m=compute_detection_metrics(te_r['labels'],te_r['scores'],pred); m.update({'model':mn,'threshold_type':'adaptive','window_size':c['threshold']['adaptive_window_size'],'k':c['threshold']['adaptive_k'],'validation_monitor_source':val_source})
