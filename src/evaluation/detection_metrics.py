...
def compute_detection_metrics(y_true, scores, y_pred):
    y=np.array(y_true).astype(int); s=np.array(scores, dtype=float); p=np.array(y_pred).astype(int)
    if not np.isfinite(s).all():
        finite = s[np.isfinite(s)]
        high = float(finite.max() + 1.0) if len(finite) else 1.0
        low = float(finite.min() - 1.0) if len(finite) else 0.0
        s = np.nan_to_num(s, nan=high, posinf=high, neginf=low)
    ...
    if len(set(y)) > 1:
        metrics['pr_auc'] = float(average_precision_score(y,s))
